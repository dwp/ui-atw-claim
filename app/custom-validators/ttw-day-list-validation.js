...
const validateJourneyNumber = require('./helpers/ttw-validation/validate-journey-number');
...
class TravelListValidation extends ValidatorFactory {
  validate(items, dataContext) {
    const monthData = dataContext.journeyContext.getDataForPage('travel-month').dateOfTravel;
    const errorMessages = [];
    ...
    Object.keys(errorMessages)
        .forEach((key) => {
          ...
          if (dataContext.journeyContext.nav.language === 'cy') {
            message.variables.dateKey = new Date(monthData.yyyy, monthData.mm -1, message.variables.dateKey).toLocaleDateString('cy', {weekday: "long", day:"numeric", month:"long"}).replace(',', '');
          } else {
